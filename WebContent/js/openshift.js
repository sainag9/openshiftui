...
mainApp.controller('controller', function($scope,$http) {
   ...
   if($scope.deproll == 'roll'){
	   ...
	   var blob_file = $scope.jenkinsfilePreview + $scope.terminaljenkins;
   } else {
...
.then(function(content) {
    ...
    $scope.preview = function(){
    	...
    	var optionTexts = [];
    	$("#sortable2 li").each(function() { optionTexts.push($(this).text()) });
    	$scope.warningflag = false;
    	if(/*$scope.github && $scope.oproject && $scope.gproject*/true){
			...
			var str1 = str.replace(/openshift_project_name/g,$scope.oproject);
			var str2 = str1.replace(/openshift_application_name/g,$scope.gproject);
			var str3 = str2.replace(/github_project_name/g,$scope.github);
			$scope.yamlfile = str3.replace(/github_secret/g,$scope.gsecret);
			...
			if($scope.emailadd){
				...
				var maill = $scope.notify_successful.replace(/replace_email/g,$scope.emailadd);
